...
def RecogniseGeneral(access_token, image=None, url=None, recognize_granularity='big', language_type='CHN',
                     detect_direction=False, detect_language=True, vertexes_location=False, probability=False):
    '''
    通用文字识别（含位置信息）
...
def anti_End(text):
    ...
    result = list()
    for letter in text:
        if letter.lower() not in 'qwertyuiopasdfghjklzxcvbnm':
            result.append(letter)
    return ''.join(result)

def Recognise(img_path):
    ak = '4ABleB4hMrtVpBUpMFptIffI'
    sk = 'RDYm31fN9ctbHcQyu4gi0bWBVLIH3Rlq'
    ...
    #     print(k, v)
    if len(str(anti_End(recognise)))!=1:
        return 0
    return recognise
